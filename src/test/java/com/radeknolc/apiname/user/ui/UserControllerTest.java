...
class UserControllerTest {
    ...
    void createUser_Authorized_Success() throws Exception {
        ...
        CreateUserRequest createUserRequest = new CreateUserRequest(username, "newuser@example.com", "Mysecretpassw0rd*");
        ...
        assertThat(response.getBody()).contains("\"username\":\"%s\"".formatted(username));
    ...
    void createUser_InvalidInputValue_HandledException(CreateUserRequest createUserRequest, List<String[]> violationData) throws Exception {
        ...
        assertThat(response.getBody()).contains("\"problemCode\":\"VALIDATION_FAILED\"");
        for (int i = 0; i < violationData.size(); i += 2) {
            assertThat(response.getBody()).contains("\"field\":\"%s\",\"message\":\"%s\"".formatted(violationData.get(i), violationData.get(i + 1)));
        }
    }
    ...
    void createUser_NotAuthenticated_HandledException() throws Exception {
        ...
        assertThat(response.getBody()).contains("\"problemCode\":\"UNAUTHORIZED\"");
    }
    ...
    void createUser_NotAuthorized_HandledException() throws Exception {
        ...
        assertThat(response.getBody()).contains("\"problemCode\":\"ACCESS_DENIED\"");
    }
